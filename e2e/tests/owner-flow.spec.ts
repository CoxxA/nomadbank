...
test('所有者可以完成核心保活流程', async ({ page }) => {
  ...
  await page.getByLabel('显示名称（可选）').fill('测试用户')
  await page.getByLabel('密码').fill('playwright-safe-password')
  ...
  for (const account of ['测试银行 A', '测试银行 B']) {
    ...
    await page.getByLabel('账户名称').fill(account)
    await page.getByLabel('分组（可选）').fill('测试分组')
  ...
  await page.getByLabel('账户分组').selectOption('测试分组')
  await page.getByLabel('生成周期').fill('1')
