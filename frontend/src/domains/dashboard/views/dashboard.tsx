...
export function Dashboard() {
  ...
  const completionRate = useMemo(() => {
    if (rangeSummary.total === null || rangeSummary.total === undefined)
      return null
    if (rangeSummary.total === 0) return 0
    return Math.round(
      ((rangeSummary.completed || 0) / rangeSummary.total) * 100
    )
  }, [rangeSummary])
        ...
        <div className='space-y-4'>
          <div className='grid gap-4 md:grid-cols-2 xl:grid-cols-4'>
            <Card className={glassCard}>
              ...
              <CardContent className='space-y-1'>
                <div className='flex items-end gap-2'>
                  <span className='font-fira-code text-3xl leading-none font-semibold'>
                    {completionRate === null ? '--' : `${completionRate}%`}
                  </span>
                  <span className='text-muted-foreground text-xs'>
                    {formatMetric(rangeSummary.completed)} /{' '}
                    {formatMetric(rangeSummary.total)}
                  </span>
                </div>
                <p className='text-muted-foreground text-xs'>
                  完成 / 总量 · {rangeMeta.label}
                </p>
              </CardContent>
            </Card>
            ...
            <Card className={glassCard}>
              ...
              <CardContent className='space-y-1'>
                <div className='font-fira-code text-3xl leading-none font-semibold'>
                  {formatMetric(rangeSummary.pending)}
                </div>
                <p className='text-muted-foreground text-xs'>
                  等待执行 · {rangeMeta.label}
                </p>
              </CardContent>
            </Card>
              ...
              <div className='flex items-start justify-between gap-4'>
                ...
                {nextDayTasks ? (
                  <div className='text-muted-foreground flex items-center gap-1 text-xs'>
                    ...
                    <span>
                      下次执行 {nextDayTasks.days_until} 天后 ·{' '}
                      {nextDayTasks.date}
                    </span>
                  </div>
                ) : null}
              ...
              {todayTasks && todayTasks.tasks.length > 0 ? (
                <div className='max-h-[200px] space-y-2 overflow-y-auto pr-1'>
                  {todayTasks.tasks.map((task) => {
                    const status =
                      ACTIVITY_STATUS[
                        task.status as keyof typeof ACTIVITY_STATUS
                      ...
                      <div key={activity.id} className='flex gap-3'>
                        <div className='flex flex-col items-center'>
                          ...
                          {index !== recentActivities.length - 1 ? (
                            <span className='bg-border/70 mt-1 w-px flex-1' />
                          ) : null}
                        </div>
      ...
      <div className='pointer-events-none fixed right-6 bottom-6 z-50'>
        <div className='pointer-events-auto flex flex-col gap-2 rounded-2xl border border-white/10 bg-white/60 p-2 opacity-90 shadow-lg backdrop-blur-lg transition-all duration-200 hover:bg-white/80 hover:opacity-100 hover:shadow-xl dark:bg-slate-900/50 dark:hover:bg-slate-900/70'>
          ...
          <Button
            ...
            disabled={
              completingAll || !todayTasks || todayTasks.pending_count === 0
            }
          >
            {completingAll ? (
              <Loader2 className='h-3.5 w-3.5 animate-spin' />
          ...
          <Button
            ...
            disabled={
              postponing || !todayTasks || todayTasks.pending_count === 0
            }
          >
            {postponing ? (
              <Loader2 className='h-3.5 w-3.5 animate-spin' />
